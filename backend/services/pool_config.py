...
def _validate_pool_top_level(pool_key: str, pool: Dict[str, Any]) -> None:
    ...
    if isinstance(source_classes, list):
        for idx, class_code in enumerate(source_classes):
            _require_str(class_code, f"{pool_key}.source_classes[{idx}]")
...
def _validate_deductions(pool_key: str, pool: Dict[str, Any]) -> Dict[str, Decimal]:
    ...
    _validate_non_negative_rate(school_rate, f"{pool_key}.deductions.school_retention.percentage")
    _validate_non_negative_rate(admin_rate, f"{pool_key}.deductions.administrative_fee.percentage")
...
def _validate_distribution(pool_key: str, pool: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if method == "attendance_weighted":
        eligible_staff = distribution.get("eligible_staff")
        if eligible_staff is None:
            raise PoolConfigError(
                f"{pool_key}.distribution.eligible_staff is required for attendance_weighted"
            )
        ...
        if eligible_staff is None:
            raise PoolConfigError(
                f"{pool_key}.distribution.eligible_staff is required for equal"
            )
        _require_str(eligible_staff, f"{pool_key}.distribution.eligible_staff")
        ...
        for index, split in enumerate(splits):
            ...
            recipients = split.get("recipients")
            split_method = split.get("split_method")

            _require_str(recipients, f"{pool_key}.distribution.splits[{index}].recipients")
            _require_str(split_method, f"{pool_key}.distribution.splits[{index}].split_method")

            if split_method not in {"equal"}:
                raise PoolConfigError(
                    f"{pool_key}.distribution.splits[{index}].split_method "
                    f"must currently be 'equal'"
                )
...
def _validate_cross_pool_rules(pools: Dict[str, Dict[str, Any]]) -> None:
    ...
    # Optional business checks for known pool codes
    if "JHS_EXTRA" in seen_codes:
...
def validate_pool_definitions(data: Dict[str, Dict[str, Any]]) -> Dict[str, Dict[str, Any]]:
    ...
    for pool_key, raw_pool in data.items():
        ...
        # Attach normalized decimal rates for easier downstream use
